...
Dim sMenu
	sMenu = "<link href='css/logis_style.css' type='text/css' rel='stylesheet' />" & vbCrLf & vbTab
	sMenu = sMenu & "<div class='contenedorMenu'>" & vbCrLf & vbTab
	sMenu = sMenu & "	<div class='dvMenu'>" & vbCrLf & vbTab
	sMenu = sMenu & "		<ul id='menu'>" & vbCrLf & vbTab
	sMenu = sMenu & "			<div class='logo-logis'>" & vbCrLf & vbTab
	sMenu = sMenu & "				<img src='images/logo-logis-s.png' style='height:50px;' />" & vbCrLf & vbTab
	sMenu = sMenu & "			</div>" & vbCrLf & vbTab
	sMenu = sMenu & "			<li onclick='window.location.href=""menu.asp"";' class='link_cursor'>" & vbCrLf & vbTab
	sMenu = sMenu & "				Inicio" & vbCrLf & vbTab
	sMenu = sMenu & "			</li>" & vbCrLf & vbTab
	sMenu = sMenu & "		</ul>" & vbCrLf & vbTab
	sMenu = sMenu & "		<h2>" & vbCrLf & vbTab
	sMenu = sMenu & "			REPORTES ANOMAL&Iacute;AS" & vbCrLf & vbTab
	sMenu = sMenu & "		</h2>" & vbCrLf & vbTab
	sMenu = sMenu & "	</div>" & vbCrLf & vbTab
	sMenu = sMenu & "</div>" & vbCrLf & vbTab
...
<body>
    ...
    <%if Request("msg") <> "" then	Response.Write "<center><table width='900px'><tr><td align='center'><font class='lblMSG green' size='2'>" & Request("msg") & "</font></td></tr></table></center><br/>"%>
...
'		response.End
	...
	if Request.Form("id_rep") = "13" then 
	    ...
	    mi_fecha = day(now) & "/" & month(now) & "/" & year(now)
                        ...
                        <td>Fecha 1 :</td>
                        <td>
                            <input type="text" name="fecha_1" class="light" size="11" readonly onclick="javascript:{if (this.value=='dd/mm/yyyy') {this.value=''}};" value="<%=mi_fecha%>" maxlength="12">
                            <%'onblur="Remplace(this.form.file_name);"%>
    ...
    FromCalendar.setOffset(5, 5);
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="fecha_2" class="light" size="11" readonly onclick="javascript:{if (this.value=='dd/mm/yyyy') {this.value=''}};" value="<%Response.Write day(now) & "/" & month(now) & "/" & year(now)%>" maxlength="12">
                        <%'onblur="Remplace(this.form.file_name);"%>
                        <script language="JavaScript" type="text/javascript">
				<!--
    //if (is_ie5up || is_nav6up || is_gecko){
    ...
    ToCalendar.setOffset(5, 5);
	    ...
	    for i = 1 to 12
                ...
                <tr>
                    <td>Rango<%=i%> :</td>
                    <td>De
                        <input type='text' name='rango_<%=i%>_from' class='light' size='11' readonly>
                        <script language='JavaScript' type='text/javascript'>
				<!--
    ...
    Rango_ <%=i %> _FromCalendar.setOffset(5, 5);
				...
				//-->
                        ...
                        A
                        <input type='text' name='rango_<%=i%>_to' class='light' size='11' readonly>
                        <script language='JavaScript' type='text/javascript'>
				<!--
		...
		<input type=text name=report_name class=light size=30 maxlength=50><br>
                ...
                <script language="javascript">
                    ...
                    function ValidateForm() {
                        ...
                        if (document.valid_conf.correo.value == "") { msg += "- no hay correo electronico.\n" };
                        if (valid_conf.num_param.value >= 1) {
                            ...
                            if (check != 0) { msg += "- los siguientes parametros : " + check + " son necesarios.\n" };
                        }


                        if (msg == "") { valid_conf.submit(); }
                        else {
                    ...
                    function Remplace(expr) {
                        ...
                        for (var i = 0; i <= new_name.length; i++) {
                            for (var j = 0; j < Forbidden_char.length; j++) {
                                if (new_name.charAt(i) == Forbidden_char.charAt(j)) {//alert (Forbidden_char.charAt(j));
                ...
                <tr>
                    <td colspan="2">
                        <br>
                        Correo electronico :
                        <input type="text" class="light" size="80" name="correo" maxlength="200" onblur="javascript:this.value=Trim(this.value);">
                        <br>
                        (se puede poner varios separados con ";" o "," )<br>
                    </td>
                </tr>
                <tr>
                    <td>
                        ...
                        <input type="hidden" name="test" value="1">
                        <input type="button" class="buttonsOrange" value="Validar" onclick="javascript:ValidateForm();" id="button1" name="button1"><br>
                        <br>
                    </td>
                </tr>
...
'<<JEMV-20220504:
	'Agrego parámetro 3 para indicar a quien se enviará el archivo TXT
	'y el parámetro 4 para indicar a quién se enviará el archivo XLS
	'en el proceso de reservación de NUI's:
	if Request.Form("id_rep") = "196" or Request.Form("id_rep") = "334" or Request.Form("id_rep") = "335" then
		SQL = SQL & "  ,PARAM_3,PARAM_4  "
	end if
' JEMV-20220504>>
...
'<<JEMV-20220504 [Internos => TXT | Helvex/Abrasivos => ]:
	'Param_3 => TXT
	'Param_4 => XLS
	
	if Request.Form("id_rep") = "196" or Request.Form("id_rep") = "334" or Request.Form("id_rep") = "335" then
		if Request.Form("param_1") = "9910" then
			SQL = SQL & ", '0','1'"
		elseif Request.Form("param_1") = "4659" or Request.Form("param_1") = "19808" or Request.Form("param_1") = "19812" or Request.Form("param_1") = "19824" or Request.Form("param_1") = "22512" or Request.Form("param_1") = "22573" or Request.Form("param_1") = "20123" then
			SQL = SQL & ", '1','1'"
		else
			SQL = SQL & ", '0','1'"
		end if
...
''''''''''''''''''''''''''' Cambio de prioridad a pedimentos Instantáneos
	...
	dim id_repd
		id_repd = Request.Form("id_rep")
	Response.Redirect "reporte_anomalia" & qa & ".asp?msg=" & Server.URLEncode("El reporte se esta generando. Checa tu correo. " & ftn_procesos_prioridad_dinamica(id_repd))
		...
		'----------------------------'Setear la prioridad de los procesos dinamicamente:----------------------------
			Function ftn_procesos_prioridad_dinamica(id_repd)
			dim bandera
				bandera = 0
			dim arrTmp
				...
				if IsArray(arrTmp) then
					for i=0 to UBound(arrTmp,2)
						if CStr(arrTmp(0,i)) = CStr(id_repd) Then
							'Response.Write "Reporte modificado de prioridad dinamicamente: "
							Response.Write "<script>console.log('Reporte modificado de prioridad dinamicamente:')</script>"
							ftn_procesos_prioridad_dinamica = " PD: " & ftn_set_prioridad(CStr(id_repd), CStr(arrTmp(1,i))) 
							bandera = 1
						end if
					next
				end if
				if bandera = 0 Then
					'Response.Write "El Reporte '" & id_repd & "' no tiene una regla para prioridad dinamica."
					Response.Write "<script>console.log(El Reporte '" & id_repd & "' no tiene una regla para prioridad dinamica."
					ftn_procesos_prioridad_dinamica = " Sin PD." 
				end if
			End Function
			Function	ftn_set_prioridad(id_repd, prioridad)
				SQL = "update rep_chron set priorite = "& prioridad &" " & VbCrLf
				SQL = SQL & "	where id_chron in ( "  & VbCrLf
				SQL = SQL & "		select id_chron from rep_chron where id_rapport in ( "  & VbCrLf
				SQL = SQL & "				select rep_det.id_cron "  & VbCrLf
				SQL = SQL & "				from rep_detalle_reporte rep_det "  & VbCrLf
				...
				SQL = SQL & "				where chron.active = 1 " & VbCrLf
				SQL = SQL & "				and chron.MINUTES is null and chron.HEURES is null and chron.JOURS is null and chron.MOIS is null and chron.JOUR_SEMAINE is null and chron.LAST_EXECUTION is null "  & VbCrLf
				SQL = SQL & "				and reporte.id_rep = "& id_repd &" "  & VbCrLf
				SQL = SQL & "			) "  & VbCrLf
				SQL = SQL & "		)"
				rst.Open SQL, Connect(), 0, 1, 1
				'Response.Write  "("& id_repd & ")"  & " -" & "- > " & prioridad  
				Response.Write "<script>console.log("& id_repd &","& prioridad &" )</script>"	
				ftn_set_prioridad = "("& id_repd & ")"  & " -" & "- > " & prioridad  
			End Function
		'-----------------------------------------------------------------------------------------------------------
        %>
        </html>
